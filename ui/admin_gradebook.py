...
Tab order: Course Gradebook → Pending Approval → Global Scheme
"""
...
def render_admin_gradebook() -> None:
    ...
    tab_course, tab_pending, tab_global = st.tabs([
        "📚 Course Gradebook",
        "✅ Pending Approval",
        "🌐 Global Scheme",
    ])

    # ── Tab 1: Course Gradebook ───────────────────────────────────
    with tab_course:
        ...
        if not sems:
            ...
            if not courses:
                ...
                sel_label   = st.selectbox("Course", list(course_options.keys()),
                                            key="adm_gb_course")
                ...
                if not enrollments:
                    ...
                    # Inner tab order: Quizzes → Assignments → Midterm → Final → Summary → Scheme
                    t_quiz, t_asgn, t_mid, t_fin, t_summary, t_scheme = st.tabs([
                        "📝 Quizzes", "📄 Assignments",
                        "📘 Midterm", "📗 Final",
                        "📊 Summary & Approve", "⚙️ Scheme",
                    ])
                    with t_quiz:
                    ...
                    with t_summary:
                        render_gradebook_summary(course_uuid, enrollments,
                                                  can_submit=True, can_approve=True)
                    with t_scheme:
    ...
    with tab_pending:
        section_header("Grades Pending Approval",
                        "Courses where faculty have submitted grades for review")
    ...
    with tab_global:
        section_header("Global Grading Scheme",
                        "Default scheme used by all courses unless overridden per-course")
        ...
        with st.form("global_scheme_form"):
            ...
            st.divider()
            section_header("Letter Grade Thresholds", "Lower bound for each letter grade")
            ...
            st.caption("F = anything below the D threshold")
